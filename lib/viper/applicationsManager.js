...
ApplicationManager.prototype.deploy = function (appId, version, app, env, file, callback) {
    this._isLoaded(function () {
            var deploy = function () {
                ...
                extract
                    ...
                    .on('finish', function () {
                        fs.readdir(appPath, function (err, files) {
                            ...
                            logger.debug('Viper', 'Package extract finding conf file', doc.appId);
                            ...
                            if (files.indexOf('package.json') !== -1) {
                                ...
                                if (!useNpm) {
                                    ...
                                    logger.debug('CAG', appId, 'npm found');
                                }
                            }
                            ...
                            function execBower(callback) {
                                async.series([
                                    ...
                                    exec('bower', ['prune'], {cwd: cache, env: self._applications[appId].metadata.env}),
                                    exec('bower', ['install'], {
                                        cwd: cache,
                                        env: self._applications[appId].metadata.env
                                    }),
                                    function (callback) {
                                        fse.copy(path.join(cache, 'bower_components'), path.join(appPath, 'bower_components'), callback);
                                    }
                                ], function (err) {
                                    callback(err);
                                });
                            ...
                            function execNpm(callback) {
                                async.series([
                                    ...
                                    exec('npm', ['prune'], {cwd: cache, env: self._applications[appId].metadata.env}),
                                    exec('npm', ['install'], {cwd: cache, env: self._applications[appId].metadata.env}),
                                    function (callback) {
                ...
                this._applications[appId].metadata.path = pathFile;
                ...
                this._db.update({appId: appId}, {
                    ...
                    this.setEnv(appId, env, function () {
                        ...
                        this._applications[appId].workersManager.reloadWorkers(function () {
                            exec('rm', ['-rf', file], function () {
                                if (oldPath) {
                                    return exec('rm', ['-rf', path.dirname(oldPath)], callback);
                                }
                                callback(null);
                            });
                        });
