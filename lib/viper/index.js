...
if (cluster.isMaster) {
    ...
    server.post('/apps/:appId', function (req, res, next) {
        ...
        applicationManager.deploy(req.params.appId, parseInt(req.body.version, 10), req.body.app, env, req.files.package.path, function (err) {
            if (err) {
...
applicationManager.on('listening', function (result) {
    if(!listening[result.appId] || listening[result.appId].port!==result.port) {
