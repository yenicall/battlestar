...
WorkersManager.prototype.spawnWorkers = function (number) {
    if (cluster.isMaster) {
        ...
        for (var n = 0; n < number; n += 1) {
            logger.debug('Master', 'Spawning worker #' + n);
            spawnWorker();
        }
        ...
        var onExit = function () {
            this.emit('exit');
            logger.info('Master', 'Exiting, killing the workers');
            for (var id in this._workers) {
                var worker = cluster.workers[id];
                logger.info('Master', 'Killing worker #' + worker.process.pid);
                worker.destroy();
            }
            process.exit(0);
        }.bind(this);
        ...
        var worker = new Worker(this.appId, db);
...
/*
 ...
 logger.debug("Master", "Shutdown complete for worker " + workerKey);
 callback();
 });
 }

 function reloadWorkers(workers) {
 logger.info('Master', '*** reloading workers!');
 var workerKey = workers.shift();

 logger.info('Master', 'restarting worker: ' + workerKey);
 ...
 logger.debug("Master", "Replacement worker online.");
 if (workers.length > 0) {
 reloadWorkers(workers);
 }
 });
 ...
 logger.debug("Master", '*** shutting down gracefully');
 var workers = Object.keys(cluster.workers);
 ...
 logger.debug('Master', 'shutting down worker: ' + workerKey);
