...
Logger.prototype.log = function (level, section) {
    ...
    this.transports.forEach(function (transport) {
        var newSection = transport.appendDate ? (new Date).toISOString() + ' [' + section + '] :' : section;
        if (transport.colorize) {
            newSection = colors[this.colors[levelPosition]](newSection);
        }
        transport.log.call(null, newSection, level, util.format.apply(null, strings));
    }.bind(this));
};
