        ...
        db.applications.findOne({appId: appId}, function (err, doc) {
            ...
            fs.exists(zip, function (exists) {
                if (exists) {
                    ...
                    logger.debug("CAG", "Finding most recent env for", doc.appId);
                    var env = {};
                    if (doc.envs) {
                        for (var i = doc.version; i > 0; i++) {
                            if (doc.envs[i]) {
                                env = doc.envs[i];
                                logger.debug("CAG", "Using env from version", i);
                                break;
                            }
                        }
                    }
    ...
    server.put('/apps/:appId', function (req, res, next) {
        ...
        db.applications.findOne({appId: req.params.appId}, function (err, doc) {
            ...
            function updateEnv() {
                ...
                updateQuery.$set['envs.' + doc.version] = req.body.env;
